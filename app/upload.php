...
//$temp_root = '/media/usb/chunk/';
$temp_root = '/media/ssd/chunk/';
...
function _log($str) {
    ...
    // log to the output
    $log_str = date('d.m.Y H:m:s').": {$str}\r\n";
    echo $log_str;

    // log to file
    if (($fp = fopen('/media/ssd/chunk/upload_log.txt', 'a+')) !== false) {
        fputs($fp, $log_str);
        fclose($fp);
    }
}
...
function createFileFromChunks($temp_dir, $fileName, $chunkSize, $totalSize,$total_files) {
    //_log($temp_dir.' '.$fileName.' '.$chunkSize.' '.$totalSize);
...
//    TO DO
    ...
    //if (isset($_GET['resumableRelativePath'])
    
    // create the final destination file 
        $shell = false;
        if ($shell == true){
            $command = 'cat '.$temp_dir.'/'.$fileName.'.part* > /media/6g_usb/upload/'.$fileName;
            _log($command);
            shell_exec($command);
            _log('merging complete');
        } else {
            if (($fp = fopen('/media/6g_usb/upload/'.$fileName, 'w')) !== false) {
                for ($i=1; $i<=$total_files; $i++) {
                    fwrite($fp, file_get_contents($temp_dir.'/'.$fileName.'.part'.sprintf('%04d',$i)));
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $temp_dir = $temp_root.$_GET['resumableIdentifier'];
    ...
    $chunk_file = $temp_dir.'/'.$_GET['resumableFilename'].'.part'.sprintf('%04d',$_GET['resumableChunkNumber']);
...
if (!empty($_FILES)) foreach ($_FILES as $file) {
    ...
    if(isset($_POST['resumableIdentifier']) && trim($_POST['resumableIdentifier'])!=''){
        $temp_dir = $temp_root.$_POST['resumableIdentifier'];
    }
    $dest_file = $temp_dir.'/'.$_POST['resumableFilename'].'.part'.sprintf('%04d',$_POST['resumableChunkNumber']);
